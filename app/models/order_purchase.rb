class OrderPurchase
  ...
  with_options presence: true do
    ...
    validates :city
    validates :addresses
    validates :phone_number, length: { minimum: 10, maximum: 11 }, format: { with: /\A0[0-9]+\z/, message: 'is invalid.' }
    validates :token
    validates :user_id
    validates :item_id
  end
  ...
  def save
    ...
    ShippingAdd.create(postal_code: postal_code, prefecture_from_id: prefecture_from_id, city: city, addresses: addresses,
                       building: building, phone_number: phone_number, purchase_record: purchase_record)
  end
end
